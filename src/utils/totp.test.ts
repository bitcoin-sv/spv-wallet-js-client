...
describe('ValidateTotpForContact', () => {
  ...
  it('should validate a TOTP for the given contact', () => {
    ...
    const result = ValidateTotpForContact(aliceClient, mockBobContact, passcode, mockBobContact.paymail);
  ...
  it('should validate a TOTP with custom period and digits', () => {
    ...
    const result = ValidateTotpForContact(aliceClient, mockBobContact, passcode, mockBobContact.paymail, 60, 2);
    expect(result).toBe(true);
  });

  it('should return false for an invalid TOTP', () => {
    const result = ValidateTotpForContact(aliceClient, mockBobContact, '24', mockBobContact.paymail);
    expect(result).toBe(false);
  });

  it('should return false for an invalid TOTP', () => {
    const result = ValidateTotpForContact(aliceClient, mockBobContact, 'avg', mockBobContact.paymail);
    expect(result).toBe(false);
  });
});
